...
상세 흐름·코드 위치는 아래 docs를 참고하세요.

- **PROFILE-EDIT-FLOW.md** — 마이페이지 전체 흐름, API 목록, 프로필 조회/수정/비밀번호/포인트/기프티콘
- **PROFILE-BLOG-POST.md** — 프로필 조회·수정 구현 정리 (블로그 포스트용)
- **BACKEND-FLOW-GUIDE.md** — 백엔드 계층 구조, 정보수정 API 요약, 파일 역할
...
- **포인트/기프티콘**: 계좌(ACCOUNT)·포인트 내역(Point_History)·주문(Order) 기준으로 조회.  
  상세: **PROFILE-EDIT-FLOW.md** 의 “포인트 내역 조회”, “기프티콘 목록 조회” 섹션.
...
- **권한**: 실제 권한 체크(관리자만 호출 가능 등)는 SecurityConfig·JWT 역할(ADMIN)로 제어.  
  자세한 경로: **BACKEND-FLOW-GUIDE.md** 참고.
